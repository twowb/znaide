...
| `/update` | 检查并更新到最新版本(`znaide --update` 同效);启动时也会自动检查一次,发现新版会提示。**双源自动回退**:默认 GitHub,网络不通(或下载失败)自动切 **Gitee 镜像源**(仓库与发布同步,国内无需代理);GitHub 源可设 `HTTPS_PROXY` 加速。Linux/macOS 替换后重启生效,Windows 退出程序后自动完成替换 |
