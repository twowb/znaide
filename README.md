...
1. **拿到二进制**,把对应平台的 `znaide`(Windows 为 `znaide.exe`)放到 `PATH` 目录,或直接使用其绝对路径。文件名带版本号(如 `znaide-linux-x64-v1.0.1`),`--version` 可确认版本;示例技能已内置,无需另装。
2. **首次启动**:在终端运行
...
└─ 状态栏:权限模式 [人格] | 模型 | 状态 · token 统计 · ctx 占用条 · 会话 <id>(各字段见下)
```
...
| `/update` | 检查并更新到 GitHub 最新版本(`znaide --update` 同效);启动时也会自动检查一次,发现新版会提示。更新走 GitHub,网络不通时设 `HTTPS_PROXY` 环境变量;Linux/macOS 替换后重启生效,Windows 退出程序后自动完成替换 |
