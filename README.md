...
3. **协议类型**:`chat`(默认,兼容最广)或 `response`(`/responses`);默认直接回车跳过
...
| `/config` | 打开配置面板(服务商 → 模型 → 协议类型 → API Key → 上下文窗口 → 轮数上限 → 验证;重开时带出当前已配置的值,顶部显示"当前配置"摘要;改完立即生效) |
...
- `providers`:**按字段覆盖内置预设**(内置条目做底,只写你要改的字段,其它字段保留内置值);每个条目支持 `base_url` / `model` / `api_key` / `api_key_env`(推荐,从环境变量读 Key)/ `context_window` / `protocol`(`chat` 默认可省略,`response` 走 `/responses`)
- 顶层 `model` / `base_url` / `api_key` / `protocol`(可选):**手动临时覆盖**,优先级高于预设;**面板保存不会写这里**。老配置里已有的顶层值会在启动时**自动搬进当前 provider 条目并清空顶层**(一次性自愈迁移),之后切换 `provider` 才真正生效
...
| `ZNAIDE_PROTOCOL` | 协议类型 `chat`(默认)/`response`(命令行 `--protocol` 优先于它) |
